  ...
  describe("💵 withdraw()", function () {
    it("Should let the owner (and nobody else) withdraw the eth from the contract...", async function () {
      ...
      const buyTokensResult = await vendor
        .connect(nonOwner)
        .buyTokens({ value: ethers.utils.parseEther("0.1") });
      console.log("\t", " 🏷  buyTokens Result: ", buyTokensResult.hash);
      ...
      console.log(
        "\t",
        " ⚖️  Starting Vendor contract ETH balance: ",
        ethers.utils.formatEther(vendorETHBalance)
      );

      console.log("\t", " 🍾 Withdrawing as non-owner (should fail)...");
      const startingNonOwnerETHBalance = await ethers.provider.getBalance(
        nonOwner.address
      );
      console.log(
        "\t",
        " ⚖️  Starting non-owner ETH balance: ",
        ethers.utils.formatEther(startingNonOwnerETHBalance)
      );
      console.log("\n    💰  Owner!\n", await vendor.owner());
      console.log("\n    💰  NONOwner!\n", nonOwner.address);
      await expect(
        //vendor.connect(nonOwner).withdraw(vendorETHBalance)
        vendor.connect(nonOwner).withdraw()
      ).to.be.revertedWith("Ownable: caller is not the owner");
      console.log("\t", " 🏷  withdraw failed with correct error");

      const newNonOwnerETHBalance = await ethers.provider.getBalance(
        nonOwner.address
      );
      console.log(
        "\t",
        " 🔎 New non-owner ETH balance: ",
        ethers.utils.formatEther(newNonOwnerETHBalance)
      );
      expect(newNonOwnerETHBalance).to.equal(startingNonOwnerETHBalance);

      console.log("\t", " 🍾 Withdrawing as owner...");
      const startingOwnerETHBalance = await ethers.provider.getBalance(
        owner.address
      );
      console.log(
        "\t",
        " ⚖️  Starting owner ETH balance: ",
        ethers.utils.formatEther(startingOwnerETHBalance)
      );
      ...
      const newOwnerETHBalance = await ethers.provider.getBalance(
        owner.address
      );
      console.log(
        "\t",
        " 🔎 New owner ETH balance: ",
        ethers.utils.formatEther(newOwnerETHBalance)
      );
      ...
      expect(newOwnerETHBalance).to.equal(
        startingOwnerETHBalance
          .add(vendorETHBalance)
          .sub(ethers.BigNumber.from(gasCost))
      );
    });
  });
